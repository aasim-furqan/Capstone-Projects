...
def insights_page():
    ...
    #3
        ...
        cursor.execute("SELECT DISTINCT State, Pincode, SUM(Registered_users) AS Users FROM top_user GROUP BY State, Pincode ORDER BY Users DESC LIMIT 10;");
        data = cursor.fetchall()
    ...
    #4
        ...
        cursor.execute("SELECT DISTINCT State, Pincode, SUM(Registered_users) AS Users FROM top_user GROUP BY State, Pincode ORDER BY Users ASC LIMIT 10");
        data = cursor.fetchall()
        columns = ['State', 'Pincode', 'Registered_users']
        df = pd.DataFrame(data, columns=columns, index=range(1, len(data) + 1))
        col1, col2 = st.columns(2)
